...
tcache_boot(tsdn_t *tsdn)
{
	 ...
	 * known.
	 */
	if (opt_lg_tcache_max < 0 || ((uint64_t)1U << opt_lg_tcache_max) < SMALL_MAXCLASS)
		tcache_maxclass = SMALL_MAXCLASS;
	else if (((uint64_t)1U << opt_lg_tcache_max) > large_maxclass)
		tcache_maxclass = large_maxclass;
	else
		tcache_maxclass = ((uint64_t)1U << opt_lg_tcache_max);
